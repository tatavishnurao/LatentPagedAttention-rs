#[cutile::module]
pub mod paged_latent_write_kernel {
    use cutile::core::*;

    #[cutile::entry()]
    pub fn paged_latent_write(
        latent_cache: &mut Tensor<f32, { [2, 8] }>,
        block_table: &Tensor<i32, { [4] }>,
        token_position: &Tensor<i32, { [1] }>,
        new_latent: &Tensor<f32, { [8] }>,
    ) {
        ...
        if physical_block_id == target_physical_block {
            ...
            let replacement: Tile<f32, { [2, 8] }> = new_latent
                .load_tile(const_shape![8], [0])
