...
pub mod gqa_decode_kernel {
    ...
    pub fn contiguous_gqa_decode(
        ...
        let k_tile: Tile<f32, { [8, 8] }> =
            k_head_major.load_tile(const_shape![8, 8], [kv_head, 0]);
        ...
        let scores: Tile<f32, { [8] }> = dot_products * scale;
        ...
        let v_tile: Tile<f32, { [8, 8] }> =
            v_head_major.load_tile(const_shape![8, 8], [kv_head, 0]);
        let probability_weights: Tile<f32, { [8, 8] }> = probabilities
