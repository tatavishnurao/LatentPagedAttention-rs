...
mod gpu_impl {
    ...
    use plkv_kernels::cutile::model_profile_preprojected::model_profile_preprojected_kernel;
    use serde::Serialize;
    ...
    const ACTIVE_SEQ_LEN: usize = 1024;
    ...
    fn enqueue_full(buffers: &mut Buffers, inputs: &Inputs, stream: &Arc<Stream>) {
        ...
        let (probabilities_part, _, _) = unsafe {
            model_profile_kernel::model_small_softmax_1024_runtime(
    ...
    fn enqueue_latent(buffers: &mut Buffers, inputs: &Inputs, stream: &Arc<Stream>) {
        ...
        let (context_part, _, _, _, _) = unsafe {
            model_profile_kernel::model_small_context_fp16_storage(
                context_out.partition([1, HEAD_DIM]),
                &probabilities,
                &inputs.latent,
                &inputs.table,
    ...
    fn enqueue_latent_preprojected(
        ...
        let (projected_part, _, _) = unsafe {
            model_profile_preprojected_kernel::model_small_project_query_once(
        ...
        let (scores_part, _, _, _, _) = unsafe {
            model_profile_preprojected_kernel::model_small_scores_fp16_storage_preprojected(
                scores_out.partition([1, BLOCK_SIZE]),
                &projected_query,
                &inputs.latent,
                &inputs.table,
