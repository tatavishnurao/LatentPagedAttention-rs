...
use plkv_core::contiguous_gqa_decode_f32;
use plkv_kernels::cutile::gqa_decode::gqa_decode_kernel;
...
fn main() {
    ...
    .expect("failed to parse GQA decode fixture");
...
fn validate_case(fixture: &Fixture, case: &Case) {
    ...
    let cpu = contiguous_gqa_decode_f32(
        &q,
        &k,
        &v,
        fixture.q_heads,
    ...
    .expect("Rust CPU contiguous GQA decode failed");
    ...
    let k_device = api::copy_host_vec_to_device(&Arc::new(k))
        .sync()
        .expect("failed to upload K")
        .reshape(&[fixture.kv_heads * fixture.seq_len, fixture.head_dim])
        .expect("failed to reshape K");
    let v_device = api::copy_host_vec_to_device(&Arc::new(v))
        .sync()
        .expect("failed to upload V")
        .reshape(&[fixture.kv_heads * fixture.seq_len, fixture.head_dim])
    ...
    let (scores_result, probabilities_result, context_result, _, _, _) =
        gqa_decode_kernel::contiguous_gqa_decode(
        ...
        .sync()
        .expect("cuTile contiguous GQA decode failed");

    let gpu_scores = scores_result
        .unpartition()
        .to_host_vec()
        .sync()
        .expect("failed to read scores");
    let gpu_probabilities = probabilities_result
        .unpartition()
        .to_host_vec()
        .sync()
        .expect("failed to read probabilities");
    let gpu_context = context_result
        .unpartition()
        .to_host_vec()
        .sync()
