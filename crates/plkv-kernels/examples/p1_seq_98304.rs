...
mod gpu_impl {
    ...
    use plkv_kernels::cutile::p1_sequence_kernels::full_kv_baseline_kernel_98304;
    use plkv_kernels::cutile::p1_sequence_kernels::model_profile_kernel_98304;
    use plkv_kernels::cutile::p1_sequence_kernels::model_profile_preprojected_kernel_98304;
    use serde::Serialize;
    ...
    const MAX_SEQ_LEN: usize = 98304;
    const ACTIVE_SEQ_LEN: usize = 98304;
    const NUM_PHYSICAL_BLOCKS: usize = 6144;
    ...
    fn enqueue_full(buffers: &mut Buffers, inputs: &Inputs, stream: &Arc<Stream>) {
        ...
        let (scores_part, _, _, _, _) = unsafe {
            full_kv_baseline_kernel_98304::model_small_full_kv_scores_fp16_storage_98304(
        ...
        let (probabilities_part, _, _) = unsafe {
            model_profile_kernel_98304::model_small_softmax_98304_runtime(
        ...
        let (context_part, _, _, _) = unsafe {
            full_kv_baseline_kernel_98304::model_small_full_kv_context_fp16_storage_98304(
    ...
    fn enqueue_latent(buffers: &mut Buffers, inputs: &Inputs, stream: &Arc<Stream>) {
        ...
        let (scores_part, _, _, _, _, _) = unsafe {
            model_profile_kernel_98304::model_small_scores_fp16_storage_98304(
                scores_out.partition([1, BLOCK_SIZE]),
                &inputs.q,
                &inputs.latent,
                &inputs.table,
        ...
        let (context_part, _, _, _, _) = unsafe {
            model_profile_kernel_98304::model_small_context_fp16_storage_98304(
                context_out.partition([1, HEAD_DIM]),
                &probabilities,
                &inputs.latent,
                &inputs.table,
    ...
    fn enqueue_latent_preprojected(
        ...
        let (projected_part, _, _) = unsafe {
            model_profile_preprojected_kernel_98304::model_small_project_query_once_98304(
        ...
        let (scores_part, _, _, _, _) = unsafe {
            model_profile_preprojected_kernel_98304::model_small_scores_fp16_storage_preprojected_98304(
                scores_out.partition([1, BLOCK_SIZE]),
                &projected_query,
                &inputs.latent,
                &inputs.table,
