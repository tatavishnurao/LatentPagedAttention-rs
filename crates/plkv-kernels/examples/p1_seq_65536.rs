...
mod gpu_impl {
    ...
    use plkv_kernels::cutile::p1_sequence_kernels::full_kv_baseline_kernel_65536;
    use plkv_kernels::cutile::p1_sequence_kernels::model_profile_kernel_65536;
    use plkv_kernels::cutile::p1_sequence_kernels::model_profile_preprojected_kernel_65536;
    use serde::Serialize;
    ...
    const ACTIVE_SEQ_LEN: usize = 65536;
    const NUM_PHYSICAL_BLOCKS: usize = 4096;
    ...
    fn enqueue_full(buffers: &mut Buffers, inputs: &Inputs, stream: &Arc<Stream>) {
        ...
        let (scores_part, _, _, _, _) = unsafe {
            full_kv_baseline_kernel_65536::model_small_full_kv_scores_fp16_storage_65536(
        ...
        let (probabilities_part, _, _) = unsafe {
            model_profile_kernel_65536::model_small_softmax_65536_runtime(
        ...
        let (context_part, _, _, _) = unsafe {
            full_kv_baseline_kernel_65536::model_small_full_kv_context_fp16_storage_65536(
    ...
    fn enqueue_latent(buffers: &mut Buffers, inputs: &Inputs, stream: &Arc<Stream>) {
        ...
        let (scores_part, _, _, _, _, _) = unsafe {
            model_profile_kernel_65536::model_small_scores_fp16_storage_65536(
                scores_out.partition([1, BLOCK_SIZE]),
                &inputs.q,
                &inputs.latent,
                &inputs.table,
        ...
        let (context_part, _, _, _, _) = unsafe {
            model_profile_kernel_65536::model_small_context_fp16_storage_65536(
                context_out.partition([1, HEAD_DIM]),
                &probabilities,
                &inputs.latent,
                &inputs.table,
    ...
    fn enqueue_latent_preprojected(
        ...
        let (projected_part, _, _) = unsafe {
            model_profile_preprojected_kernel_65536::model_small_project_query_once_65536(
        ...
        let (scores_part, _, _, _, _) = unsafe {
            model_profile_preprojected_kernel_65536::model_small_scores_fp16_storage_preprojected_65536(
                scores_out.partition([1, BLOCK_SIZE]),
                &projected_query,
                &inputs.latent,
                &inputs.table,
