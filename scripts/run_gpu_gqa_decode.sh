...
out="reports/rtx4060_gpu_smoke/gqa_decode_${stamp}.txt"

{
  echo "LatentPagedAttention-rs RTX 4060 contiguous GQA decode validation"
  ...
  cargo run \
    ...
    -p plkv-kernels \
    --features gpu-cutile \
    --example gqa_decode_gpu
} 2>&1 | tee "$out"

grep -q "GQA_DECODE_GPU_OK=1" "$out"
