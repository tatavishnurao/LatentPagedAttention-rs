...
out="reports/rtx4060_gpu_smoke/paged_lookup_${stamp}.txt"

{
  echo "LatentPagedAttention-rs RTX 4060 paged lookup validation"
  ...
  cargo run \
    ...
    -p plkv-kernels \
    --features gpu-cutile \
    --example paged_lookup_gpu
} 2>&1 | tee "$out"

grep -q "PAGED_LOOKUP_GPU_OK=1" "$out"
echo "GPU_REPORT=$out"
