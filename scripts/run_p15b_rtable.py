...
Runs the p15b_rtable example for each sequence length with the
...
def build(seq: int) -> None:
    ...
    if binary.exists():
        return
    print(f"Building p15b_rtable (once for all lengths)...", flush=True)
...
def run_length(seq: int, warmup: int, iterations: int) -> dict:
    ...
    for line in record["stdout_tail"].splitlines():
        if line.startswith("P15B_SUMMARY_PATH="):
    ...
    elif "P15B_OK=1" in record["stdout_tail"]:
        record["status"] = "OK"
    else:
